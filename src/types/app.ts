...
// ── Active workout state (from Claude-ui.md) ──────────────────────────────────
...
export interface ActiveExerciseState {
  ...
  sets:               SetEntry[];
  lastPerformanceSets: SetValues[] | null; // null = first time doing this exercise
...
export interface ActiveWorkoutState {
  ...
  isCompleting: boolean; // true while complete-workout API call is in flight
}

// ── Legacy active workout (used by workout-store.ts until prompt 007/008) ─────
// Kept for backward-compatibility with the Prompt 001 scaffold store.
...
export interface StartWorkoutExercise {
  sessionExercise: SessionExerciseRow;
  exercise:        ExerciseWithSchema;
  /** Raw sets_data from last_performance_snapshots — use .values for display */
...
export interface HistorySessionSummary {
  ...
  total_sets:       number;   // completed sets only
  volume_kg:        number;   // sum of weight × reps for completed sets
}
