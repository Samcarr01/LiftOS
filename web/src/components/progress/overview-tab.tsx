...
/** "Overview" tab: AI coaching report + supporting 30-day stats and charts. */
        ...
        <>
          ...
          {ai && (isCoachingFormat || isLegacySections) ? (
            <>
              {ai.greeting && (
                <div className="content-card border-primary/20 bg-primary/[0.06]">
                  <div className="flex items-start gap-3">
                    ...
                    <p className="text-base font-medium leading-relaxed pt-1">{ai.greeting}</p>
                  </div>
                </div>
              )}
                ...
                <div className="content-card space-y-5">
                  {ai.whats_working && (
                    <div>
                      ...
                      <p className="text-sm leading-relaxed text-foreground">{ai.whats_working}</p>
                    </div>
                  )}
                  {ai.improving_on && (
                    <div>
                      <p className="text-overline mb-2">Where to Improve</p>
                      <p className="text-sm leading-relaxed text-foreground">{ai.improving_on}</p>
                    </div>
                  )}
                  {ai.getting_stronger && (
                    <div>
                      <p className="text-overline mb-2">Are You Getting Stronger?</p>
                      <p className="text-sm leading-relaxed text-foreground">{ai.getting_stronger}</p>
                    </div>
                  )}
                </div>
              )}
              ...
              {ai.sign_off && (
                <p className="text-center text-sm italic text-muted-foreground px-4">{ai.sign_off}</p>
              )}
            </>
          ) : ai ? (
            ...
            <>
              ...
              {summary.insight && (
                <div className="content-card">
                  ...
                  <p className="mt-2 text-sm leading-relaxed text-muted-foreground">{summary.insight}</p>
                </div>
              )}
            </>
          )}
          ...
          {/* ─── ACTION BAR ─── */}
          {generating && ai && <GeneratingCard generating />}
          <div className="flex gap-2">
            {ai && (
              <button onClick={handleCopy} className="premium-button-secondary flex-1 justify-center">
                {copied ? (
